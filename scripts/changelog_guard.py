...
COMMIT_PREFIX_RE = re.compile(r"\b(?:feat|fix|refactor|chore|test|perf|docs|build|ci|style)(?:\([^\n)]*\))?:")
...
METADATA_LINE_RE = re.compile(r"(?:^|>\s*|-\s*)(?:生成时间|工具：|工具:|后端变更\s*\d+\s*个文件|前端变更\s*\d+\s*个文件|仓库列表|统计信息)")
...
def validate_file(file_path, order, check_tech):
    ...
    for line_no, raw_line in enumerate(lines, start=1):
        ...
        if METADATA_LINE_RE.match(line):
            errors.append(f"第 {line_no} 行：检测到生成元信息 `{line}`；请删除统计信息、生成时间、工具名或仓库说明")
            continue
        ...
        if check_tech and line.startswith("- "):
            commit_match = COMMIT_PREFIX_RE.search(line)
            if commit_match:
                token = commit_match.group(0)
                errors.append(f"第 {line_no} 行：检测到 commit 风格表述 `{token}`；请改写成产品动作或业务变化，不要直接照抄提交说明")
